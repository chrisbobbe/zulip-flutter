...
`git sync-fork`. A stale fork `main` also widens the range the
`authorship` suite examines in a cloud session, where the fork's
`main` stands in for upstream's.
...
- **Setup script**: paste this one line:
  ...
  script's working directory isn't reliably the checkout. If
  your fork isn't named `zulip-flutter`, the checkout directory
  presumably isn't either; adjust the path to match.)
